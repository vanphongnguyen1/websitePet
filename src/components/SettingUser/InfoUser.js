...
const InfoUser = ({ dataOrder }) => {
  ...
  const onFinish = values => {
    const id = sessionStorage.getItem('id')
    customAxiosApi.put(`${API_NAME.USERS}/${id}`, values)
      .then(resp => {
        const { data } = resp
  ...
  return (
    <>
              ...
              !isDisabled && (
                <>
                  ...
                  <Form.Item {...tailLayout}>
                    ...
                    <Button type="primary" danger onClick={() => setIsDisable(true)}>
                      Hủy bỏ
                    </Button>
                  </Form.Item>
                </>
      ...
      {
        dataOrder.length ? (
          <p className="info-user__text"> Bạn đã mua {dataOrder.length} đơn hàng. </p>
        ) : (
